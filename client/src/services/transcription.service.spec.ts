...
/** Mock de AxiosInstance con solo los métodos usados por transcriptionService */
const createMockApi = (): Pick<AxiosInstance, "post" | "get" | "delete"> => ({
  post: vi.fn(),
  get: vi.fn(),
  delete: vi.fn(),
});
...
describe("transcription.service", () => {
  ...
  describe("upload", () => {
    it("llama POST /transcriptions/upload con el request", async () => {
      mockApi.post.mockResolvedValue({
        data: { id: "t1", status: "pending", uploadUrl: "https://s3.example.com", expiresIn: 3600 },
      });

      const result = await transcriptionService.upload(mockApi as AxiosInstance, {
        fileName: "audio.mp3",
        fileSize: 1024,
      });
  ...
  describe("list", () => {
    it("llama GET /transcriptions con params", async () => {
      ...
      await transcriptionService.list(mockApi as AxiosInstance, { page: 2, pageSize: 20 });
  ...
  describe("getStats", () => {
    it("llama GET /transcriptions/stats", async () => {
      ...
      const result = await transcriptionService.getStats(mockApi as AxiosInstance);

      expect(mockApi.get).toHaveBeenCalledWith("/transcriptions/stats");
      expect(result).toEqual({ totalBatchSeconds: 100, totalRealtimeSeconds: 50 });
    });
  });
  ...
  describe("delete", () => {
    it("llama DELETE /transcriptions/:id", async () => {
      ...
      await transcriptionService.delete(mockApi as AxiosInstance, "trans-1");
  ...
  describe("getRealtimeSession", () => {
    it("llama POST /transcriptions/realtime", async () => {
      ...
      const result = await transcriptionService.getRealtimeSession(mockApi as AxiosInstance);
  ...
  describe("uploadWithConfirmation", () => {
    it("sube a presigned URL y confirma", async () => {
      ...
      expect(mockApi.post).toHaveBeenCalledWith("/transcriptions/upload", expect.any(Object));
      ...
      expect(mockApi.post).toHaveBeenCalledWith("/transcriptions/t1/confirm", { duration: 120 });
    ...
    it("lanza error si fetch falla", async () => {
      ...
      mockApi.post.mockResolvedValueOnce({
        data: { id: "t1", uploadUrl: "https://s3.example.com", status: "pending", expiresIn: 3600 },
      });

      const file = new File(["x"], "a.mp3", { type: "audio/mpeg" });
      await expect(
        transcriptionService.uploadWithConfirmation(mockApi as AxiosInstance, file)
  ...
  describe("download", () => {
    it("crea link de descarga y hace click", async () => {
      ...
      await transcriptionService.download(mockApi as AxiosInstance, "trans-1", "audio.mp3");

      expect(mockApi.get).toHaveBeenCalledWith("/transcriptions/trans-1/download", {
        responseType: "blob",
      });
    });
  });
